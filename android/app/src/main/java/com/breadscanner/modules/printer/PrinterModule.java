...
public class PrinterModule extends ReactContextBaseJavaModule {
    ...
    @ReactMethod
    public void Sam4sStartPrint(String portNumber, String van, String finalOrderData, String itemData,String payResultData, String businessData, String storeName, String orderNo) {
        ...
        if(!itemData.equals("")) {
            try {
                ...
                JSONObject bData = null;

                if (van.equals(KOCES)) {
                    bData = new JSONObject(businessData);
                ...
                if (jsonArray.length() > 0) {
                    ...
                    if(drivers.size()>0) {
                        ...
                        int newPortNumber = Integer.parseInt(portNumber);

                        if(result.size() >= (newPortNumber+1) ) {

                            mUSBPrinterInfo = (USBPrinterInfo) new USBPrinterInfo(result.get(newPortNumber));
                            ...
                            if(retBool) {
                                ...
                                try {
                                    ...
                                    getReactApplicationContext().runOnUiQueueThread(new Runnable() {
                                        @Override
                                        public void run() {
                                            Toast.makeText(getReactApplicationContext(),"프린터를 연결할 수 없습니다.",Toast.LENGTH_SHORT).show();
                                        }
                                    });
                                }
                            }else {
    ...
    public int PrintData(Sam4sBuilder builder) {
        int iret = -1;
        Log.d("SAM4S", String.valueOf(mManager.isConnected()));
